...
import { Search, Check, X, RotateCcw, Zap, Lock } from "lucide-react";
...
const ErrorDetectivePage = () => {
  ...
  useEffect(() => { if (settings.errorDetectiveEnabled) setCases(getShuffledErrors()); }, []);

  if (!settings.errorDetectiveEnabled) {
    return (
      <PageLayout title="Error Detective">
        <section className="py-20 px-4 text-center">
          ...
          <p className="text-muted-foreground font-body text-sm">Error Detective has been disabled by the admin.</p>
        </section>
      </PageLayout>
    );
  }
  ...
  const checkAnswer = () => {
    if (!selected) return;
    setChecked(true); setTotal(prev => prev + 1);
  ...
  const next = () => {
    if (caseIdx + 1 >= cases.length) { setCases(getShuffledErrors()); setCaseIdx(0); }
    else setCaseIdx(prev => prev + 1);
        ...
        <div className="container mx-auto max-w-2xl">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            <h2 className="font-display text-2xl md:text-3xl font-bold text-center neon-text mb-2">
              <Search className="inline w-7 h-7 text-primary mr-2" /> Error <span className="text-primary">Detective</span>
            </h2>
          ...
          <div className="glass-card-strong p-6 mb-6">
            ...
            <pre className="bg-background/50 rounded-lg p-4 font-mono-code text-sm text-neon-orange whitespace-pre-wrap">{current.code}</pre>
          </div>

          <div className="grid grid-cols-2 gap-3 mb-6">
            {ERROR_TYPES.map(type => (
              <motion.button key={type} onClick={() => { if (!checked) setSelected(type); }}
                whileHover={!checked ? { scale: 1.03 } : {}} whileTap={!checked ? { scale: 0.97 } : {}}
                className={`glass-card p-4 text-sm font-body font-semibold transition-all ${
                  checked && type === current.errorType ? "border-neon-green/50 bg-neon-green/10 text-neon-green"
                    : checked && type === selected && !isCorrect ? "border-destructive/50 bg-destructive/10 text-destructive"
                    : selected === type ? "border-primary/50 text-primary" : "text-foreground hover:border-primary/30"
                }`} disabled={checked}>
                {type}
              </motion.button>
            ))}
