...
import { GripVertical, Check, X, RotateCcw, Puzzle, Zap } from "lucide-react";
...
const PhaseChallengePage = () => {
  ...
  const loadChallenge = useCallback(() => {
    ...
    if (!result) {
      ...
      if (fresh) {
        ...
        setItems(shuffleArray(fresh.challenge.steps));
        setUsedIndices([fresh.index]);
      }
    } else {
      setChallenge(result.challenge);
      setItems(shuffleArray(result.challenge.steps));
  ...
  useEffect(() => { loadChallenge(); }, []); // eslint-disable-line
      ...
      <section className="py-10 px-4">
        <div className="container mx-auto max-w-2xl">
          ...
          <div className="flex gap-3 mt-6 justify-center">
            {!checked ? (
              <button onClick={checkOrder} className="neon-btn text-sm inline-flex items-center gap-2">
                <Check className="w-4 h-4" /> Check Order
              </button>
            ) : (
              <button onClick={loadChallenge} className="neon-btn neon-btn-cyan text-sm inline-flex items-center gap-2">
          ...
          <AnimatePresence>
            {checked && (
              ...
              >
                {correct ? (
                  <p className="font-display text-neon-green text-sm">
                    <Zap className="inline w-4 h-4 mr-1" /> Perfect! +200 points
                  </p>
                ) : (
                  <div>
